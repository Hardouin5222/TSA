...
def build_offer_from_supplier_template(
    *,
    ...
    normalized_fare_options = [normalize_fare_option(option) for option in raw_fare_options]
    capabilities = build_offer_capabilities(normalized_fare_options)
    ...
    return FlightOffer(
        ...
        fare_options=normalized_fare_options,
        capabilities=capabilities,
        supplier_context=SupplierContext(
            source_mode=settings.flight_data_mode.lower(),
            provider_code=str(template.get("provider", "")).lower().replace(" ", "_") or None,
            provider_name=str(template.get("provider")) if template.get("provider") else None,
        ),
    )
...
def normalize_fare_option(option: dict) -> FlightOffer.FareOption:
    service_flags = FareServiceFlags(
        seat_selection=bool(option.get("seat_selection")),
        meal_selection=bool(option.get("meal_included")),
        refundable=bool(option.get("refundable")),
        exchangeable=bool(option.get("exchangeable")),
    )

    return FlightOffer.FareOption(
        id=str(option["id"]),
        label=str(option["label"]),
        badge=str(option["badge"]) if option.get("badge") is not None else None,
        price_delta=float(option.get("price_delta", 0)),
        hand_baggage=str(option.get("hand_baggage", "")),
        checked_baggage=str(option.get("checked_baggage", "")),
        features=[str(feature) for feature in option.get("features", [])],
        seat_selection=service_flags.seat_selection,
        refundable=service_flags.refundable,
        exchangeable=service_flags.exchangeable,
        meal_included=service_flags.meal_selection,
        service_flags=service_flags,
    )


def build_offer_capabilities(fare_options: list[FlightOffer.FareOption]) -> SupplierOfferCapabilities:
    has_multiple_brands = len(fare_options) > 1
    has_checked_baggage_upgrade = any(
        option.checked_baggage and option.checked_baggage.lower() not in {"bagaj yok", "0", "0 kg"} for option in fare_options[1:]
    )

    return SupplierOfferCapabilities(
        branded_fares_supported=has_multiple_brands,
        checked_baggage_upsell_supported=has_checked_baggage_upgrade,
        seat_selection_supported=any(option.service_flags.seat_selection for option in fare_options),
        meal_selection_supported=any(option.service_flags.meal_selection for option in fare_options),
        traveler_note_supported=any(
            option.service_flags.seat_selection or option.service_flags.meal_selection for option in fare_options
        ),
        price_calendar_supported=False,
        hold_supported=False,
        ticketing_supported=True,
        live_pricing_supported=settings.flight_data_mode.lower() != "mock_supplier",
    )
