...
export function ResultsList({ data }: { data: FlightSearchEnvelope["data"] }) {
  ...
  function shouldOpenPackageModal(offer: FlightOffer) {
    return offer.capabilities?.branded_fares_supported || offer.fare_options.length > 1;
  }
  ...
  async function addFlightOfferToCart(offer: FlightOffer) {
  ...
  async function handleDirectSelection(offer: FlightOffer) {
    const selectedFareOption = offer.fare_options[0];
    if (!selectedFareOption) {
      return;
    }

    await addFlightOfferToCart(buildOfferFromFareOption(offer, selectedFareOption));
  }
  ...
  async function handleConfirmFareSelection() {
    ...
    await addFlightOfferToCart(buildOfferFromFareOption(activeOffer, selectedFareOption));
  }
                    ...
                    <article className={`turna-offer-card${index === 0 ? " is-featured" : ""}`} key={offer.id}>
                      <div className="turna-offer-main">
                        ...
                        <div className="turna-offer-foot">
                          <button
                            className="turna-expand-chip"
                            onClick={() => (shouldOpenPackageModal(offer) ? openPackageModal(offer) : void handleDirectSelection(offer))}
                            type="button"
                          >
                            +
                          </button>
